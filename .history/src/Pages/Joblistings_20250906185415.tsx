...
const JobListings: React.FC = () => {
      ...
      (entries) => {
        if (entries[0].isIntersecting && hasMore) {
          ...
          setFilteredJobs((prev) => [
            ...prev,
            ...jobs.slice(prev.length, prev.length + 10),
          ]);
          if (jobs.length <= filteredJobs.length + 10) setHasMore(false);
        }
      },
      { threshold: 0.1 }
    );
  ...
  useEffect(() => {
    ...
    setFilteredJobs(filtered.slice(0, page * 10));
  }, [
    jobs,
    searchTitle,
    selectedCategories,
              ...
              filteredJobs.map((job: JobType, index: number) => (
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-3 sm:gap-4">
                    ...
                    {user && job.userId === user._id ? ( // Changed user.id to user._id
                      <>
                        <motion.button
                          whileHover={{ scale: 1.05 }}
                          whileTap={{ scale: 0.95 }}
                      ...
                      <motion.button
                        ...
                        whileTap={{ scale: 0.95 }}
                        onClick={() => navigate(`/job-details/${job.id}`)}
                        className="px-6 sm:px-8 py-2 sm:py-3 bg-gradient-to-r from-green-600 to-green-700 text-white font-bold rounded-xl hover:from-green-700 hover:to-green-800 transition-all shadow-lg text-sm sm:text-base font-inter"
                      >
                        🚀 Apply Now
                      </motion.button>
                    ) : null}
