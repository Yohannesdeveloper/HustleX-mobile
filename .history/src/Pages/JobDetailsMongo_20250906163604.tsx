...
interface JobPost {
  ...
  company?: string;
  budget: string;
  duration?: string; // Made optional to align with API
  category: string;
  jobType?: string; // Made optional to align with API
  workLocation?: string; // Made optional to align with API
  experience?: string;
  education?: string;
  gender?: string;
  vacancies?: number;
...
const JobDetailsMongo: React.FC = () => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      if (apiService.isAuthenticated()) {
        try {
          const userFromApi = await apiService.getCurrentUser(); // API response

          // Map MongoDB _id to id and ensure profile exists
          const user: User = {
            id: userFromApi._id, // required by User type
  ...
  useEffect(() => {
    const checkApplication = async () => {
      ...
      try {
        // Fetch the application for the current user and job
        const application: Application | null =
          await apiService.checkApplication(job._id);

        // Set applied directly using boolean
          ...
          {userRole === "freelancer" &&
            ...
            job.postedBy?._id !== currentUser?.id && (
              <motion.div
                ...
                } backdrop-blur-xl border rounded-3xl p-8 mt-8 shadow-[0_4px_6px_rgba(0,0,0,0.3)] max-w-2xl mx-auto`}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.5 }}
              >
                <h2
                  className={`text-3xl font-bold mb-6 flex items-center gap-2 font-inter ${
                    darkMode ? "text-cyan-400" : "text-cyan-600"
                  }`}
                >
