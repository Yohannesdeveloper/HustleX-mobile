...
const JobDetailsMongo: React.FC = () => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      if (apiService.isAuthenticated()) {
        try {
          const userFromApi = await apiService.getCurrentUser(); // type: User from API

          // Map MongoDB _id to id to satisfy TypeScript User type
          const user: User = {
            ...userFromApi,
            id: userFromApi._id, // ensure 'id' exists
          };
  ...
  useEffect(() => {
    const checkApplication = async () => {
      ...
      try {
        const response = (await apiService.checkApplication(
          job._id
