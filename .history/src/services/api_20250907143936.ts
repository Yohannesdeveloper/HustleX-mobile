...
class ApiService {
  ...
  constructor() {
    // Detect if running on a forwarded tunnel (Cursor/DevTunnels)
  ...
  // Added methods for password reset functionality
  async sendPasswordResetOTP(email: string): Promise<void> {
    try {
      await axios.post(`${this.baseUrl}/auth/send-otp`, { email });
    } catch (error) {
      throw new Error(
